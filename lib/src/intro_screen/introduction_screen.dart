...
import 'package:flutter_keyboard_visibility/flutter_keyboard_visibility.dart'; // TODO

import '../intro_sliding/animated_background_color.dart';
import 'helper.dart';
...
class IntroductionScreenState extends State<IntroductionScreen> {
  ...
  Future<void> _movePage(
    ...
    await Future<void>.delayed(autoscrollDuration);
    // FIXED : added mounted check
    if (!_isSkipPressed && !_isScrolling && mounted) {
      if (forward) {
  ...
  Widget build(BuildContext context) {
    final bool isLastPage = (getCurrentPage() == getPagesLength() - 1);
    ...
    return SafeArea(
      ...
      child: Scaffold(
        ...
        body: AnimatedBackgroundColor(
          ...
          pageCount: widget.pages != null ? widget.pages!.length : widget.rawPages!.length,
            ...
            children: <Widget>[
                ...
                child: NotificationListener<ScrollNotification>(
                  ...
                  child: PageView(
                    ...
                    children: widget.pages
                            ?.mapIndexed(
                              (int index, PageViewModel page) => IntroPage(
                                page: page,
              ...
              if (_showBottom)
                Positioned(
                  ...
                  child: (MediaQuery.of(context).size.width > 720) // FIXME
                      ? landscape(leftBtn, rightBtn)
                      : portrait(leftBtn, rightBtn),
                ),
            ],
          ),
        ),
  ...
  Widget portrait(Widget? leftBtn, Widget? rightBtn) {
    return Column(
      children: <Widget>[
        pageController(leftBtn, rightBtn),
        if (widget.globalFooter != null) widget.globalFooter!,
      ],
    );
  }

  Widget landscape(Widget? leftBtn, Widget? rightBtn) {
    return Row(
      children: <Widget>[
        Flexible(
          flex: 3,
          child: pageController(leftBtn, rightBtn),
        ),
        Flexible(
  ...
  Widget pageController(Widget? leftBtn, Widget? rightBtn) {
    return Container(
                  ...
                  ? widget.customProgress ??
                      Semantics(
                        ...
                        child: DotsIndicator(
                          ...
                          onTap: widget.isProgressTap && !widget.freeze ? (int pos) => animateScroll(pos) : null,
                        ),
                      )
                  : const SizedBox(),
            ),
