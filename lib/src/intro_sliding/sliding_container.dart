...
class _SlidingContainer extends State<SlidingContainer> {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: _notifier,
      builder: (BuildContext context, Widget? anim) {
        double offsetX = (_notifier.value - _notifier.value.toInt()) * widget.offset;
        offsetX *= -1;
        final bool isCurrentPage = _notifier.value.toInt() == _page;
